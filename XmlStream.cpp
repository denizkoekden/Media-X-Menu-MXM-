...
void XmlStream::terminateNode( string & name, string & attributes )
{
	// if no name stop
	if ( name.empty() )
		return;
	...
	// tell subscriber
	if ( _subscriber )
		_subscriber->terminateNode(name,attributes);
}
...
bool XmlStream::parseNodes ( XmlParser & parser, char * buffer, long parseLength )
{
	...
	if ( showDebugInfo )
	{
		cout << "parse node: " << parser.m_lID << endl;
		cout << endl;
	}
	...
	if ( showDebugInfo )
	{
		cout << "parse node: " << parser.m_lID << "ENDED" << endl;
		cout << endl;
	}
