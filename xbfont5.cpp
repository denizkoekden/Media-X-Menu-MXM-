...
// Static objects
//-----------------------------------------------------------------------------
DWORD CXBFontFive::m_dwFontVertexShader = 0L;
DWORD CXBFontFive::m_dwFontPixelShader  = 0L;


CCaratInfo::CCaratInfo(void) : 
	m_iCaratPos(-1), 
	m_iStyle(1),
	m_dwOutlineColor(0xffffffff),
	m_dwFillColor(0xffffff00),
	m_bDraw(false)
{
}

CCaratInfo::~CCaratInfo()
{
}

DWORD CCaratInfo::GetAdjustedFillColor( void )
{
	DWORD dwReturn = m_dwFillColor&0x00ffffff;
	DWORD dwAlpha = GetTickCount();
...
DWORD CCaratInfo::GetAdjustedOutlineColor( void )
{
	DWORD dwReturn = m_dwOutlineColor;
	DWORD dwAlpha = GetTickCount();
	dwAlpha &= 0xff;
...
HRESULT DrawCarat( IDirect3DDevice8* pDevice, RECT * pRect,
							...
							int iType  )
{
	...
	if ( pRect )
	{
		x1 = (FLOAT)(pRect->left+1);
		x2 = (FLOAT)(pRect->right-1);
		y1 = (FLOAT)(pRect->top+pRect->bottom);
		y1 /= 2.0f;
		y2 = (FLOAT)(pRect->bottom-3);

		switch( iType )
		{
			default:
			case 1: // Normal, on character
				v[0] = D3DXVECTOR4( x1, y2, TWOD_Z, 0 );
				v[1] = D3DXVECTOR4( x2, y1, TWOD_Z, 0 );
				v[2] = D3DXVECTOR4( x2, y2, TWOD_Z, 0 );
				break;
			case 0: // First position
				v[0] = D3DXVECTOR4( x1, y1, TWOD_Z, 0 );
				v[1] = D3DXVECTOR4( x2, y2, TWOD_Z, 0 );
				v[2] = D3DXVECTOR4( x1, y2, TWOD_Z, 0 );
				break;
		}
...
//-----------------------------------------------------------------------------
HRESULT CXBFontFive::CreateShaders()
{
...
//-----------------------------------------------------------------------------
HRESULT CXBFontFive::Create( LPDIRECT3DDEVICE8 pd3dDevice, const CHAR* strFontResourceFileName, DWORD dwResourceOffset )
{
	m_pd3dDevice = pd3dDevice;
    // Load the resource for the font
// LAL TEMP COMMENT OUT
    if( FAILED( m_xprResource.Create( pd3dDevice, strFontResourceFileName, 2 ) ) )
        return E_FAIL;
...
HRESULT CXBFontFive::CreateFromMem( LPDIRECT3DDEVICE8 pd3dDevice, LPCTSTR szSection, 
					const LPBYTE pDataBuffer )
{
	m_pd3dDevice = pd3dDevice;
    if( FAILED( m_xprResource.CreateFromMem( pd3dDevice, szSection, pDataBuffer, 
                                      2 ) ) )
        return E_FAIL;

	// dwResourceOffset
	return Create( m_xprResource.GetTexture( (DWORD)0 ), // dwResourceOffset ), 
                   m_xprResource.GetData( (DWORD)0 + sizeof(D3DTexture) ) );
}
...
// Desc: Create the font's internal objects (texture and array of glyph info)
//-----------------------------------------------------------------------------
HRESULT CXBFontFive::Create( D3DTexture* pFontTexture, VOID* pFontData )
{
...
//$TODO - remove this old way of doing fonts
        ...
        for( WORD i=0; i<=m_cMaxGlyph; i++ )
        {
            ...
            else
            {
                ...
                GLYPH_ATTR5* pGlyph = &m_Glyphs[m_TranslatorTable[i]];
...
//$END_TODO - remove this old way of doing fonts
    ...
    else if( dwFileVersion == 0x00000005 )
    {
        ...
        pData += sizeof(DWORD);
        m_Glyphs = (GLYPH_ATTR5*)pData;
    }
    else
    {
        OUTPUT_DEBUG_STRING( "XBFontFive: Incorrect version number on font file!\n" );
        return E_FAIL;
    }
...
//-----------------------------------------------------------------------------
VOID CXBFontFive::SetCursorPosition( FLOAT fCursorX, FLOAT fCursorY )
{
...
//-----------------------------------------------------------------------------
VOID CXBFontFive::SetScaleFactors( FLOAT fXScaleFactor, FLOAT fYScaleFactor )
{
...
//-----------------------------------------------------------------------------
VOID CXBFontFive::SetSlantFactor( FLOAT fSlantFactor )
{
...
//-----------------------------------------------------------------------------
HRESULT CXBFontFive::GetTextExtent( const WCHAR* strText, FLOAT* pWidth, 
...
//-----------------------------------------------------------------------------
FLOAT CXBFontFive::GetTextWidth( const WCHAR* strText ) const
{
...
//-----------------------------------------------------------------------------
HRESULT CXBFontFive::DrawText( DWORD dwColor, const WCHAR* strText, DWORD dwFlags,
                           FLOAT fMaxPixelWidth, CCaratInfo * pCaratInfo )
{
    return DrawText( m_fCursorX, m_fCursorY, dwColor, strText, 
                     dwFlags, fMaxPixelWidth, pCaratInfo );
}
...
// Desc: Draws text as textured polygons
//-----------------------------------------------------------------------------
HRESULT CXBFontFive::DrawText( FLOAT fOriginX, FLOAT fOriginY, DWORD dwColor,
                           ...
                           FLOAT fMaxPixelWidth, CCaratInfo * pCaratInfo )
{
    ...
    while( *strText )
    {
        ...
        // Translate unprintable characters
        GLYPH_ATTR5* pGlyph = &m_Glyphs[ (letter<=m_cMaxGlyph) ? m_TranslatorTable[letter] : 0 ];
		...
		if ( iCaratPos == 0 )
		{
			if ( pCaratInfo )
			{
				pCaratInfo->m_rcCaratRect.left = (int)(left1+0.5);
				pCaratInfo->m_rcCaratRect.right = (int)(right1+0.5);
				pCaratInfo->m_rcCaratRect.top = (int)(top+0.5);
				pCaratInfo->m_rcCaratRect.bottom = (int)(bottom+0.5);
...
//-----------------------------------------------------------------------------
LPDIRECT3DTEXTURE8 CXBFontFive::CreateTexture( const WCHAR* strText, 
...
void	CXBFontFive::CleanString( CStdString & sString )
{
}

void	CXBFontFive::WrapString( CStdString & sString, int iPixelWidth )
{
}
