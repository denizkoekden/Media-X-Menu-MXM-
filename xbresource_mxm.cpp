...
HRESULT CXBPackedResource::CreateFromMem( LPDIRECT3DDEVICE8 pd3dDevice, LPCTSTR szSection, const LPBYTE pDataBuffer, 
                    DWORD dwNumResources, XBRESOURCE* pResourceTags )
{
    VOID *pLoc = NULL;
	HRESULT hr = S_OK;
	LPBYTE pDataBufferPtr;
	...
	if ( pLoc && pDataBuffer )
	{
		...
		memcpy( &xprh, pDataBuffer, sizeof(XPR_HEADER) );
		pDataBufferPtr = (LPBYTE)pDataBuffer;
		pDataBufferPtr += sizeof(XPR_HEADER);
	//    ReadFile(hFile, &xprh, sizeof(XPR_HEADER), &dwNumBytesRead, NULL);
		if( xprh.dwMagic != XPR_MAGIC_VALUE )
		{
			OUTPUT_DEBUG_STRING( "Invalid Xbox Packed Resource" );
			hr = E_INVALIDARG;
		}
		if ( hr == S_OK )
		{
			...
			memcpy( m_pSysMemData, pDataBufferPtr, dwSysMemDataSize );
			pDataBufferPtr += dwSysMemDataSize;
			memcpy( m_pVidMemData, pDataBufferPtr, dwVidMemDataSize );
